...
ExynosJpegEncoderForCamera::ExynosJpegEncoderForCamera(bool bBTBComp)
{
    ...
    m_JpegMainEncodeThread = new MJpegThread(this,
            &ExynosJpegEncoderForCamera::m_JpegMainEncodeThreadFunc, "JpegMainEncodeThread", PRIORITY_URGENT_DISPLAY);


    m_ionJpegClient = createIonClient(m_ionJpegClient);
    if(m_ionJpegClient == 0) {
        ALOGW("WARN(%s):ionJpegClient initialization failed. Will be tried again on use.", __FUNCTION__);
    }
}
...
ExynosJpegEncoderForCamera::~ExynosJpegEncoderForCamera()
{
    ...
    if(m_ionJpegClient!= 0) {
        ALOGE("ERR(%s):Cannot deinitialize m_ionJpegClient [%d]"
            , __FUNCTION__, (int)m_ionJpegClient);
    }
}

bool ExynosJpegEncoderForCamera::m_JpegMainEncodeThreadFunc(void)
{
    if (!m_jpegMain->encode()) {
	return false;
	ALOGE("encode() fail");
	}
    return true;
...
int ExynosJpegEncoderForCamera::create(void)
{
    ...
    if (m_jpegMain == NULL) {
        ...
        if (m_jpegMain == NULL) {
            ALOGE("ERR(%s):Cannot create ExynosJpegEncoder class", __FUNCTION__);
    ...
    if(m_ionJpegClient == 0) {
        /*
         It was supposed to be initialized on the constructor,
         but if it it was failed, tried again here.
         */
        ALOGW("WARN(%s):Retrying ionJpegClient initialization...", __FUNCTION__);
        m_ionJpegClient = createIonClient(m_ionJpegClient);

        if(m_ionJpegClient == 0)
            return ERROR_CANNOT_CREATE_EXYNOS_JPEG_ENC_HAL;

        ALOGI("INFO(%s):ionJpegClient initialization succeeded.", __FUNCTION__);
    }
...
int ExynosJpegEncoderForCamera::destroy(void)
{
    ...
    if (m_jpegThumb != NULL) {
        ...
        // May be those memory can be reused...
        freeJpegMemory(&m_stThumbInBuf, MAX_IMAGE_PLANE_NUM);
        freeJpegMemory(&m_stThumbOutBuf, MAX_IMAGE_PLANE_NUM);

        /* In original design, m_ionJpegClient is deinitialized here.
           But now it is moved to the destructor to avoid
           open/close overhead in create() and destroy() method */
        //m_ionJpegClient = deleteIonClient(m_ionJpegClient);

        m_stThumbInBuf.ionClient = m_stThumbOutBuf.ionClient = 0;
        m_jpegThumb->destroy();
        delete m_jpegThumb;
        m_jpegThumb = NULL;
    }
...
int ExynosJpegEncoderForCamera::setOutBuf(int buf, int size)
{
    ...
    if (ret) {
        ALOGE("ERR(%s):Fail to JPEG output buffer!!", __FUNCTION__);
        return ret;
    }
...
int ExynosJpegEncoderForCamera::encode(int *size, exif_attribute_t *exifInfo, char** pcJpegBuffer,debug_attribute_t *debugInfo)
{
    ...
    m_JpegMainEncodeThread->run(PRIORITY_URGENT_DISPLAY);
    ALOGI("JPEG main encoder start");

    int iOutputSize = 0;

    //int iJpegBuffer = 0;

    if (!(m_jpegMain->checkInBufType() & JPEG_BUF_TYPE_USER_PTR) && !(m_jpegMain->checkInBufType() & JPEG_BUF_TYPE_DMA_BUF)) {
	...
	bool thumba = false;
        ...
        ALOGD("AppMarker exif(without thumb) size = %d", exifLen);
...
#if 0
        if (debugInfo != NULL && debugInfo->debugData != NULL) {
            SecAppMarker *m_debugMarker;
            m_debugMarker = new SecAppMarker;
            m_debugMarker->setExifStartAddr((char *)exifOut, exifLen);
            m_debugMarker->setDebugStartAddr(debugInfo->debugData, debugInfo->debugSize);
            m_debugMarker->pushDebugData();
	...
	if (iJpegSize<=0) {
        ...
        ALOGD("AppMarker jpeg size = %d", iJpegSize);

        if (exifLen <= EXIF_LIMIT_SIZE) {
            ALOGI("INFO(%s[%d]): memmove size:%d exifLen=%d", __FUNCTION__, __LINE__, iJpegSize - 2,exifLen);
            memmove(*pcJpegBuffer + exifLen + 2, *pcJpegBuffer + 2, iJpegSize - 2);
            ALOGI("INFO(%s[%d]): memmove - end", __FUNCTION__, __LINE__);

	    ALOGI("try memcmp exifLen=%d",exifLen);
            if (exifLen <= bufSize) {
                memcpy(*pcJpegBuffer + 2, exifOut, exifLen);
                iJpegSize += exifLen;
		//int oldiJpegSize = iJpegSize;
	/*	if ( iJpegSize >= 2 ) {
		     int tmpiJpegSize = iJpegSize -1;
		     do {
		     	if (*(char*)(*pcJpegBuffer+tmpiJpegSize) == 0xd9
			   && *(char*)(*pcJpegBuffer+(tmpiJpegSize-1)) == 0xff)
		     	break;
		        iJpegSize = tmpiJpegSize;
			tmpiJpegSize--;
		     }
		     while (tmpiJpegSize>0);
		     if (oldiJpegSize != (tmpiJpegSize+1))
			ALOGE("JPEG returned %zu but EOI ends at %zu",oldiJpegSize,tmpiJpegSize+1);
		}*/
            } else {
    ...
    ALOGI("End encode - iJpegSize=%d",iJpegSize);
    *size = iJpegSize;
                              ...
                              bool useMainbufForThumb)
{
    ...
    memmove(exifInfo->user_comment + sizeof(code), exifInfo->user_comment, exifInfo->user_comment_size);
    memcpy(exifInfo->user_comment, code, sizeof(code));
    writeExifIfd(&pCur, EXIF_TAG_USER_COMMENT, EXIF_TYPE_UNDEFINED,
                 exifInfo->user_comment_size + sizeof(code), exifInfo->user_comment, &LongerTagOffest, pIfdStart);
    ...
    if (exifInfo->enableThumb) {
		...
		ALOGI("DEBUG(%s) thumbSize=%d",__FUNCTION__,thumbSize);
            }

            if (mmapJpegMemory(&iThumbFd, &thumbBuf, &thumbBufSize, MAX_OUTPUT_BUFFER_PLANE_NUM) == false) {
                ALOGE("ERR(%s): mmapJpegMemory() fail", __FUNCTION__);
    ...
    if (exifInfo->enableThumb && (thumbBuf != NULL) && (thumbSize != 0)) {
        ...
        if (LongerTagOffest > EXIF_LIMIT_SIZE) {
            ALOGE("ERR(%s):ExifTagOffset(%d) is too bigger than EXIF_LIMIT_SIZE(%d)",
                  __FUNCTION__, LongerTagOffest, EXIF_LIMIT_SIZE);
...
int ExynosJpegEncoderForCamera::scaleDownYuv422(char **srcBuf, unsigned int srcW, unsigned int srcH,  char **dstBuf, unsigned int dstW, unsigned int dstH)
{
    ...
    for (unsigned int y = 0; y < dstH; y++) {
        ...
        for (unsigned int x = 0; x < dstW; x += 2) {
            ...
            if ((unsigned int)src_pos >= srcW * srcH * 2) break;
...
int ExynosJpegEncoderForCamera::encodeThumbnail(unsigned int *size, bool useMain)
{
    ...
    if (m_jpegThumb == NULL) {
        ...
        if (m_jpegThumb == NULL) {
            ALOGE("ERR(%s):Cannot open a jpeg device file", __FUNCTION__);
    ...
    if (ret) {
        ALOGE("ERR(%s):Fail create", __FUNCTION__);
        return ret;
    }

        ret = m_jpegThumb->setCache(JPEG_CACHE_ON);
    if (ret) {
        ALOGE("ERR(%s):Fail cache set", __FUNCTION__);
        return ret;
    }

    void *pConfig = m_jpegMain->getJpegConfig();
    if (pConfig == NULL) {
        ALOGE("ERR(%s):Fail getJpegConfig", __FUNCTION__);
        return ERROR_BUFFR_IS_NULL;
    }

    ret = m_jpegThumb->setJpegConfig(pConfig);
    if (ret) {
        ALOGE("ERR(%s):Fail setJpegConfig", __FUNCTION__);
        return ret;
    }

    ret = m_jpegThumb->setSize(m_thumbnailW, m_thumbnailH);
    if (ret) {
        ALOGE("ERR(%s):Fail setSize", __FUNCTION__);
        return ret;
    }
    ...
    if (ret) {
        ALOGE("ERR(%s):Fail setInBuf", __FUNCTION__);
        return ret;
    }
    ...
    if (ret) {
        ALOGE("ERR(%s):Fail setOutBuf", __FUNCTION__);
        return ret;
    }
    ...
    if (useMain) {
        ...
        int iMainInputBuf[MAX_INPUT_BUFFER_PLANE_NUM] = {0,};
        char *pcMainInputBuf[MAX_INPUT_BUFFER_PLANE_NUM] = {NULL,};
        char *pcThumbInputBuf[MAX_INPUT_BUFFER_PLANE_NUM] = {NULL,};
        int iMainInputSize[MAX_INPUT_BUFFER_PLANE_NUM] = {0,};
        int iThumbInputSize[MAX_INPUT_BUFFER_PLANE_NUM] = {0,};
        ...
        if (ret) {
            ALOGE("ERR(%s):Fail getSize", __FUNCTION__);
            return ret;
        }
        ...
        if (ret) {
            ALOGE("ERR(%s):Fail getInBuf", __FUNCTION__);
            return ret;
        }
        ...
        case V4L2_PIX_FMT_YUYV:
            if (m_jpegMain->checkInBufType() & JPEG_BUF_TYPE_DMA_BUF) {
                /* Consider HW capability of scaling down */
                if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    && (iTempWidth <= (m_thumbnailW * 16))
                    && (iTempHeight <= (m_thumbnailH * 16))) {
                    ALOGD("DEBUG(%s):scale down by csc : iTempWidth(%d)(ALIGN_DOWN(iTempWidth,16)(%d)), iTempHeight(%d)(ALIGN_DOWN(iTempHeight,16)(%d)), m_thumbnailW(%d), m_thumbnailH(%d)",
                            __FUNCTION__, iTempWidth, iTempHeight, ALIGN_DOWN(iTempWidth,16), ALIGN_DOWN(iTempHeight,16), m_thumbnailW, m_thumbnailH);
                    ...
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s) dmabuf : csc_convert() from gralloc to callback fail", __FUNCTION__);
                } else {
                    ALOGD("DEBUG(%s):scale down by scaleDownYuv422 : iTempWidth(%d), iTempHeight(%d), m_thumbnailW(%d), m_thumbnailH(%d)",
                        __FUNCTION__, iTempWidth, iTempHeight, m_thumbnailW, m_thumbnailH);
                ...
                if ((m_exynosThumbCSC && ALIGN_DOWN(iTempWidth, 16) == iTempWidth)
                    ...
                    if (csc_convert(m_exynosThumbCSC) != 0)
                        ALOGE("ERR(%s) userptr : csc_convert() from gralloc to callback fail", __FUNCTION__);
                } else {
                    ret = scaleDownYuv422(pcMainInputBuf,
                                          iTempWidth,
                                          iTempHeight,
        ...
        if (ret) {
            ALOGE("ERR(%s):scaleDown(%d, %d, %d, %d) fail", __FUNCTION__, iTempWidth, iTempHeight, m_thumbnailW, m_thumbnailH);
            return ret;
        }
    }
    else {
    ...
    do {
        ...
        if (ret) {
            ALOGE("ERR(%s):Fail setQuality", __FUNCTION__);
            return ret;
        }
    ...
    *size = (unsigned int)iOutSizeThumb;
    ...
    ALOGI("Encode thumbnail end!! iOutSizeThumb=%d",iOutSizeThumb);

    return ERROR_NONE;
}

int ExynosJpegEncoderForCamera::createIonClient(int ionClient)
{
    if (ionClient == 0) {
        ionClient = ion_open();
        if (ionClient < 0) {
            ALOGE("[%s]src ion client create failed, value = %d", __FUNCTION__, ionClient);
            return 0;
        }
    }
...
int ExynosJpegEncoderForCamera::deleteIonClient(int ionClient)
{
    if (ionClient != 0) {
        if (ionClient > 0)
            ion_close(ionClient);
...
int ExynosJpegEncoderForCamera::allocJpegMemory(struct stJpegMem *pstMem, int iMemoryNum)
{
    ...
    if (pstMem->ionClient == 0) {
        ALOGE("[%s] i = %d , ionClient is zero (%d)", __FUNCTION__, i, pstMem->ionClient);
        return ERROR_BUFFR_IS_NULL;
    }
    ...
    for (i=0;i<iMemoryNum;i++) {
        ...
        if (ion_alloc_fd(pstMem->ionClient, pstMem->iSize[i], 0, ION_HEAP_SYSTEM_MASK, 0, &pstMem->ionBuffer[i]) <= -1) {
            ALOGE("[%s]ion_alloc(%d) failed", __FUNCTION__, pstMem->iSize[i]);
            pstMem->ionBuffer[i] = -1;
            freeJpegMemory(pstMem, iMemoryNum);
            return ERROR_MEM_ALLOC_FAIL;
        }

        pstMem->pcBuf[i] = (char *)mmap(NULL,pstMem->iSize[i],PROT_READ | PROT_WRITE, MAP_SHARED, pstMem->ionBuffer[i], 0);
        if ((pstMem->pcBuf[i] == (char *)MAP_FAILED) || (pstMem->pcBuf[i] == NULL)) {
            ALOGE("[%s]src ion map failed(%d)", __FUNCTION__, pstMem->iSize[i]);
    ...
    for (i=0;i<iMemoryNum;i++) {
        if (pstMem->ionBuffer[i] != -1) {
            if (pstMem->pcBuf[i] != (char *)MAP_FAILED)
                munmap(pstMem->pcBuf[i], pstMem->iSize[i]);

            close(pstMem->ionBuffer[i]);
        }
        pstMem->ionBuffer[i] = -1;
        pstMem->pcBuf[i] = (char *)MAP_FAILED;
        pstMem->iSize[i] = 0;
...
bool ExynosJpegEncoderForCamera::mmapJpegMemory(int *iFd, char **ppcBuf, int *piSize, int iMemoryNum)
{
    ...
    for (int i = 0; i < iMemoryNum; i++) {
        if (piSize[i] != 0) {
            ppcBuf[i] = (char *)mmap(NULL, piSize[i], PROT_READ | PROT_WRITE, MAP_SHARED,iFd[i],0);
            if ((ppcBuf[i] == (char *)MAP_FAILED) || (ppcBuf[i] == NULL)) {
                ALOGE("[%s]ion map failed(size[%u])", __FUNCTION__, (unsigned int)piSize[i]);
