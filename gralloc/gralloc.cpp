                             ...
                             unsigned int ion_flags, private_handle_t **hnd, int *stride)
{
    ...
    internal_format = format;//gralloc_select_format(format, usage, is_compressible);
	...
	ion_flags = ((unsigned int)usage >> 24) & 8 | ((unsigned int)usage >> 3) & 0x40;
                             ...
                             private_handle_t **hnd, int *stride)
{
    ...
    *stride = ALIGN(w, 16);
    ...
    ALOGI("allocate yuv usage=%#X ", usage);
