                 ...
                 void** vaddr)
{
    ...
    *vaddr = INT_TO_PTR(hnd->base);
    ...
    if (hnd->format == HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SPN)
        vaddr[1] = (void **)(char *)vaddr[0] + (hnd->stride * hnd->vstride) + ext_size;
    else if (hnd->format == HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SPN_S10B)
        vaddr[1] = (void **)(char *)vaddr[0] + (hnd->stride * hnd->vstride) + ext_size + (ALIGN(hnd->width / 4, 16) * hnd->vstride) + 64;
