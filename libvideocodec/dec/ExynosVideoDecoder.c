...
static void *MFC_Decoder_Init(ExynosVideoInstInfo *pVideoInfo)
{
    ...
    if (pCtx->nPrivateDataShareFD <= 0) {
        ALOGE("%s: Failed to ion_alloc nPrivateDataShareFD", __func__);
        goto EXIT_QUERYCAP_FAIL;
    }

    pCtx->pPrivateDataShareAddress =
    ...
    if (pCtx->pPrivateDataShareAddress == MAP_FAILED) {
        ALOGE("%s: Failed to ion_map nPrivateDataShareFD", __func__);
        goto EXIT_QUERYCAP_FAIL;
    }
...
static ExynosVideoErrorType MFC_Decoder_Finalize(void *pHandle)
{
    ...
    if (pCtx->pPrivateDataShareAddress != NULL) {
        ion_unmap(pCtx->pPrivateDataShareAddress, sizeof(PrivateDataShareBuffer) * VIDEO_BUFFER_MAX_NUM);
    ...
    ExynosVideoInstInfo *pVideoInstInfo)
{
    ...
    if (pVideoInstInfo->eCodecType == VIDEO_CODING_HEVC) {
        if (pVideoInstInfo->eSecurityType == VIDEO_SECURE) {
            hDec = exynos_v4l2_open_devname(VIDEO_HEVC_SECURE_DECODER_NAME, O_RDWR, 0);
            if (hDec < 0)
    ...
    {
        if (pVideoInstInfo->eSecurityType == VIDEO_SECURE) {
            hDec = exynos_v4l2_open_devname(VIDEO_MFC_SECURE_DECODER_NAME, O_RDWR, 0);
            if (hDec < 0)
