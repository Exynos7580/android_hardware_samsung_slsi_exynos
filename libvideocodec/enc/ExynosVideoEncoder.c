...
static void __Set_SupportFormat(ExynosVideoInstInfo *pVideoInstInfo)
{
    ...
    case MFC_80:
        ...
        pVideoInstInfo->supportFormat[nLastIndex++] = VIDEO_COLORFORMAT_YV12;
...
static ExynosVideoErrorType MFC_Encoder_Set_QuantizationRange(
    void *pHandle, int QpMin, int QpMax)
{
    ExynosVideoEncContext *pCtx = (ExynosVideoEncContext *)pHandle;
    ExynosVideoErrorType   ret  = VIDEO_ERROR_NONE;
    ...
    case VIDEO_CODING_AVC:
    {
        ...
        maxId = V4L2_CID_MPEG_VIDEO_H264_MAX_QP;
        minId = V4L2_CID_MPEG_VIDEO_H264_MIN_QP;
        break;
    }
    case VIDEO_CODING_MPEG4:
    {
        ...
        maxId = V4L2_CID_MPEG_VIDEO_MPEG4_MAX_QP;
        minId = V4L2_CID_MPEG_VIDEO_MPEG4_MIN_QP;
        break;
    }
    case VIDEO_CODING_H263:
    {
        ...
        maxId = V4L2_CID_MPEG_VIDEO_H263_MAX_QP;
    ...
    case VIDEO_CODING_VP8:
    {
        ...
        maxId = V4L2_CID_MPEG_VIDEO_VP8_MAX_QP;
    ...
    if (exynos_v4l2_s_ctrl(pCtx->hEnc, maxId, QpMax) != 0) {
        ALOGE("%s: Failed to s_ctrl", __func__);
        ret = VIDEO_ERROR_APIFAIL;
        goto EXIT;
    }
    if (exynos_v4l2_s_ctrl(pCtx->hEnc, minId, QpMin) != 0) {
        ALOGE("%s: Failed to s_ctrl", __func__);
        ret = VIDEO_ERROR_APIFAIL;
        goto EXIT;
    }
...
static ExynosVideoErrorType MFC_Encoder_Enqueue_All_Outbuf(void *pHandle)
{
    ...
    unsigned long   dataSize[VIDEO_BUFFER_MAX_PLANES] = {0, };
    ...
    ExynosVideoInstInfo *pVideoInstInfo)
{
    ...
    if (pVideoInstInfo->eSecurityType == VIDEO_SECURE) {
        hEnc = exynos_v4l2_open_devname(VIDEO_SECURE_ENCODER_NAME, O_RDWR, 0);
        if (hEnc < 0)
